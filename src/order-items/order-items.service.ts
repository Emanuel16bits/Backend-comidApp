...
export class OrderItemsService {
  ...
  async create(createOrderItemDto: CreateOrderItemDto): Promise<OrderItem> {
    const orderItem = this.orderItemRepository.create(
      createOrderItemDto as DeepPartial<OrderItem>,
    );
  ...
  async findAll(): Promise<OrderItem[]> {
    return await this.orderItemRepository
      ...
      .leftJoinAndSelect('orderItem.orden', 'orden')
      .leftJoinAndSelect('orderItem.producto', 'producto')
      .select([
        'orderItem',
        'orden.id',
        'orden.precioTotal',
  ...
  async remove(id: number): Promise<{ message: string }> {
    ...
    return { message: `OrderItem con id ${id} eliminado` };
  }
}
